from __future__ import division   #精确算法 除  而不是给一个整数
import os
import numpy as np
from  imageio import imread  #读一张图片的属性，返回   np.array、uint8、255、HWC、RGB

import torch
import torch.utils.data as data

from datasets import pms_transforms  #中断去写  pms_transforms
from . import util  #中断去仅仅创建该文件，、
np.random.seed(0)
...
class PS_Synth_Dataset(data.Dataset):
    ...
    def _getInputPath(self,index):
        ...
        normal_path = os.path.join(self.root,'Images',shape,shape + '_normal.png')   #法向图路径
        img_dir    = os.path.join(self.root,'Images',self.shape_list[index])   #图像文件夹路径
        img_list   = util.readList(os.path.join(img_dir,'%s_%s.txt' % (shape,mtrl)))

        data = np.genfromtxt(img_list,dtype='str',delimiter=' ') # dtype 最终数据类型，delimiter 数据分割的标志
        select_idx = np.random.permutation(data.shape[0])[:self.args.in_img_num] #生成随机序列，【32】个
        idxs = ['%03d' % (idx) for idx in select_idx]
        data = data[select_idx, :]
        imgs = [os.path.join(img_dir,img) for img in data[:, 0]]  #第一（0）列的所有数据，也就是图片名称
        lights = data[:,1:4].astype(np.float32) #变化数组类型  1 2 3 列是光照方向
