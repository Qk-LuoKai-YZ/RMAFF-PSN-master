...
def calNormalAcc(gt_n,pred_n,mask=None): # 计算误差的
    dot_product = (gt_n*pred_n).sum(1).clamp(-1,1)# clamp()   缩小到【-1,1】
    error_map = torch.acos(dot_product)
    angular_map = error_map*180.0 /math.pi
    angular_map = angular_map*mask.narrow(1,0,1).squeeze(1)  # .narrow 取mask的数据干什么？
